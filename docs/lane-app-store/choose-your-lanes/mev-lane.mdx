...
If you have not already, we recommend following the [General Setup]/block-sdk/integrate) guide first!
...
The `MEVLane` requires that apps use the `LanedMempool` as their app-side mempool, see [here]/block-sdk/integrate)
